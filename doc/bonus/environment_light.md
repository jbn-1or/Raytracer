...
**原因**：Quad 在相机后方。当视线方向与 Quad 的法线方向一致时，光线参数 `t` 可能为负。

**排查**：检查 Quad 的 z 坐标是否在相机前方，确保 `t > 0`。

### 照片上下翻转

**原因**：Quad 的 v 向量方向与 `ImageTexture` 内部的 `v = 1.0 - v` 翻转约定不匹配。

**修复**：将 v 向量设为 `(0, 3, 0)`（向上），起点 Q 设为左下角，这样 UV 的 `(0,0)` 在左下、`(1,1)` 在右上，与图像坐标系一致。

### 发光偏暗

**原因**：`ImageTexture::value()` 将像素值乘以 `1.0/255.0`，返回的颜色在 `[0, 1]` 范围内，作为光源时可能不够亮。
