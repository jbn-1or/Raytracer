...
use crate::tools::material::{Lambertian, Material};
...
pub fn render() {
    let path = prepare_output_path("output/book1/image19.png");
    ...
    let r = (PI / 4.0).cos();
    // 左侧
    let left_material: Arc<dyn Material> = Arc::new(Lambertian::new(Color::new(0.0, 0.0, 1.0)));
    ...
    // 右侧
    let right_material: Arc<dyn Material> = Arc::new(Lambertian::new(Color::new(1.0, 0.0, 0.0)));
    world.add(Box::new(Sphere::new_with_material(
        Point3::new(r, 0.0, -1.0),
        r,
        right_material,
