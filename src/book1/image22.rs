...
pub fn render() {
    let path = prepare_output_path("output/book1/image22.png");
    ...
    // 左侧
    let left_material: Arc<dyn Material> = Arc::new(Dielecric::new(1.50));
    world.add(Box::new(Sphere::new_with_material(
    ...
    let bubble_meterial: Arc<dyn Material> = Arc::new(Dielecric::new(1.00 / 1.50));
    world.add(Box::new(Sphere::new_with_material(
        Point3::new(-1.0, 0.0, -1.0),
        0.4,
        bubble_meterial,
    ...
    // 右侧：金色金属材质
    let right_material: Arc<dyn Material> =
        Arc::new(Metal::new_with_fuzz(Color::new(0.8, 0.6, 0.2), 1.0));
    world.add(Box::new(Sphere::new_with_material(
        Point3::new(1.0, 0.0, -1.0),
        0.5,
        right_material,
